...
##To-do:
    # Fix for the 'continue fix' of missing data
    ...
    # Legend is not replaced on replotting
    # Solved - Add option to colour lines based on month of the year
...
class GraphPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        selected.set("Select")
        colour_check_var = tk.IntVar()
        plot_type_var = tk.IntVar()
        ...
        def dropdown_func():
            ...
            for file in glob.glob(file_path + selected_mooring + '/*.nc'):
                ...
                ######('continue' stops current loop iteration and moves onto next file)
                ...
                if fv == 1:
                    ...
                    #Clean 'bad data'
                    temp_qc = df.TEMP_quality_control
                    pres_qc = df.PRES_REL_quality_control
                    sal_qc = df.PSAL_quality_control
                    dox_qc = df.DOX_quality_control
                    par_qc = df.PAR_quality_control
                    cphl_qc = df.CPHL_quality_control
                    columns_qc = [temp_qc, pres_qc, sal_qc, dox_qc, par_qc, cphl_qc]
                    ...
                    #Scatter plots
                    # if plot_type_var.get() == 1:
                    #     temp_plt.scatter(temp, pres, s=5, label=profile_date, color=monthly_colors)
                    #     sal_plt.scatter(sal, pres, s=5, color=monthly_colors)
                    #     dox_plt.scatter(dox, pres, s=5, color=monthly_colors)
                    #     par_plt.scatter(par, pres, s=5, color=monthly_colors)
                    #     cphl_plt.scatter(cphl, pres, s=5, color=monthly_colors)
                    #     f.legend(title="Legend", prop={'size': 7}, loc=1)
                    # elif plot_type_var.get() == 0:
                    #     temp_plt.scatter(temp, pres, s=5, label=profile_date)
                    #     sal_plt.scatter(sal, pres, s=5)
                    #     dox_plt.scatter(dox, pres, s=5)
                    #     par_plt.scatter(par, pres, s=5)
                    #     cphl_plt.scatter(cphl, pres, s=5)
                    #     f.legend(title="Legend", prop={'size': 7}, loc=1)
        ...
        mooringList = ('HIS', 'MYR', 'LSL', 'PPS', 'CCH', 'HOL', 'OTE')
        dropdown = tk.OptionMenu(frame, selected, *mooringList)
        dropdown.place(relwidth=0.2, relheight=0.075, relx=0.35, rely=0.1)
        colour_checkbox = tk.Checkbutton(frame, text='Monthly Colours', variable=colour_check_var)
        colour_checkbox.place(relwidth=0.15, relheight=0.04, relx=0.7, rely=0.1)
        plot_type_var = tk.Checkbutton(frame, text='Line/Scatter', variable=plot_type_var)
        plot_type_var.place(relwidth=0.15, relheight=0.04, relx=0.7, rely=0.15)
...
>>>>>>> 2ececb4427634f27c46a14491ae36e328e8f5738:gui_prac.py
